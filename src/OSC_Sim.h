...
namespace Excimontec {
	...
	//! \author Michael C. Heiber and contributors
	//! \date 2017-2020
	...
	public:
		...
		//! \brief Checks whether the specified STT state file contains valid data.
		//! \param state_filename specified the path to the STT state file.
		//! \return true if the STT state file contains valid data.
		//! \return false if the STT state file does not contain all the required data.
		bool checkSTTStateFile(const std::string state_filename) const;
		...
		//! \brief Exports the relative or absolute lattice site energies to a text file.
		//! \param filename is the name of the data file that will be created in the working directory.
		//! \param relative is an optional parameter that can be set to false to output absolute site energies.
		//! \param charge is an optional parameter designates whether electron or hole energies are calculated when absolute energies are output.
		void exportEnergies(const std::string filename, const bool relative = true, const bool charge = true);
